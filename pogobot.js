...
var preMsg = {};
var msgCnt = 0;
let longspace = "\u200b".repeat(500); // 공백 추가
...
function response(room, msg, sender, isGroupChat, replier, imageDB)
{
   ...
   if (msg.indexOf("어디") > -1 || msg.indexOf("어딘") > -1 || msg.indexOf("어딩") > -1 || msg.indexOf("어딧") > -1 || msg.indexOf("어딜") > -1
      || msg.indexOf("오디") > -1 || msg.indexOf("오딘") > -1 || msg.indexOf("오딩") > -1 || msg.indexOf("오딧") > -1 || msg.indexOf("오딨") > -1
      ...
      || msg.indexOf("@") == 0) 
   {
      ...
      else 
      {
         ...
         {
            if (msg.indexOf(gymData[i][0]) > -1) // 이름1과 같은게 있으면
            {
               if (msg.indexOf(gymData[i][3]) == -1) 
               {
                  if (gymData[i][1] != "-") 
                  {
                     if (gymData[i][2] != "-") 
                     {
                        ...
                        replier.reply("▷"+gymData[i][8]+" 위치정보"+longspace+"\n"+gymData[i][2]+"\n\r"+gymData[i][8]+"의 체육관 위치는 아래와 같습니다.\n\r (치킨)카카오맵\nhttps://map.kakao.com/link/map/"+gymData[i][6]+","+gymData[i][1]+"\n\r (맥주)구글맵\n"+"https://www.google.com/maps/q="+gymData[i][1]+"\n");
                     }
                     else
                     {
                        replier.reply("▷"+gymData[i][8]+" 위치정보"+longspace+"\n\r"+gymData[i][8]+"의 체육관 위치는 아래와 같습니다.\n\r (치킨)카카오맵\nhttps://map.kakao.com/link/map/"+gymData[i][6]+","+gymData[i][1]+"\n\r (맥주)구글맵\n"+"https://www.google.com/maps?q="+gymData[i][1]+"\n");
               ...
               else
               {
                  if (gymData[i][4] != "-") 
                  {
                     if (gymData[i][5] != "-")
                     {
                        ...
                        replier.reply("▷"+gymData[i][8]+" 위치정보"+longspace+"\n"+gymData[i][5]+"\n\r"+gymData[i][8]+"의 체육관 위치는 아래와 같습니다.\n\r (치킨)카카오맵\nhttps://map.kakao.com/link/map/"+gymData[i][7]+","+gymData[i][4]+"\n\r (맥주)구글맵\n"+"https://www.google.com/maps/q="+gymData[i][4]+"\n");
                     }
                     else
                     {
                        replier.reply("▷"+gymData[i][8]+" 위치정보"+longspace+"\n\r"+gymData[i][8]+"의 체육관 위치는 아래와 같습니다.\n\r (치킨)카카오맵\nhttps://map.kakao.com/link/map/"+gymData[i][7]+","+gymData[i][4]+"\n\r (맥주)구글맵\n"+"https://www.google.com/maps?q="+gymData[i][4]+"\n");
   ...
   if (msg.indexOf("@") == 0)
   {
      ...
      if (msg.indexOf("둥지") == 1)
      {
         ...
         replier.reply("(치킨)둥지정보 안내(치킨)" + longspace + "\n (치킨)다크라이 홈페이지\n\r https://darkrai.synology.me:9999/pokemongo.html\n\r (하트)inven 홈페이지\n\r http://www.inven.co.kr/board/pokemongo/4985/4476\n");
         return;
      }
      ...
      if (msg.indexOf("레이드") == 1)
      {
         replier.reply("https://drive.google.com/file/d/1jKspD8y2dYq94Go0b-49Hzlnmkyn8Swe/view?usp=sharing");
         return;
      }
